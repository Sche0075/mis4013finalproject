...
function selectAlbums() {
    try {
        ...
        $stmt = $conn->prepare("SELECT al.album_id, a.artist_name, g.genre_name, al.album_name, al.album_year FROM artist a JOIN album al ON al.artist_id = a.artist_id JOIN genre g ON g.genre_id = al.genre_id");
        $stmt->execute();
